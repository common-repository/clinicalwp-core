    ...
    class Clinical_Browser_Cache_Plugin
    {
        ...
        function Clinical_alt_caching_write_file( $file_path ){
            ...
            if (is_writable($file_path)) {
                $fp = fopen($file_path, "a");
                if (flock($fp, LOCK_EX, $wouldblock)) {  //get file lock
                    $date = new DateTime( date('D, d M Y H:i:s') ); //added 'D, d M Y H:i:s' - check doesnt cause issues
        ...
        function Clinical_enable_maintenance_write_file( $file_path ){
            ...
            if (is_writable($file_path)) {
                $fp = fopen($file_path, "a");
                if (flock($fp, LOCK_EX, $wouldblock)) {  //get file lock
                    fwrite($fp, "# START Clinical CMS Enable Maintenance\n");
        ...
        public static function Clinical_caching_erase_file($backup, $backup_filename, $method){
            ...
            if($backup) {
                if(copy ( ABSPATH . '.htaccess' , ABSPATH . $backup_filename ) && $method != 6) {
                    $status = true;
                } else if(copy ( ABSPATH . 'wp-config.php' , ABSPATH . $backup_filename ) && $method == 6) {
                    $status = true;
                }
                else {
                    $status = false;
