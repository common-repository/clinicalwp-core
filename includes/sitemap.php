...
Modyfied and updated by Phill Healey @ CodeClinic.de on 01.08.2015
	Original Plugin Name: Alley Sitemaps
	Original Plugin URI: http://www.alleyinteractive.com/
	Version: 0.1
	Original Author: Matthew Boynes
*/
...
class Alley_Sitemap {
	...
	public function setup() {
        ...
        add_action('wp_head', array( $this, 'create_reference' ), 5 );
	}
	/**
	 * Add sitemap reference in page head meta
	 *
	 * @return void
	 */
    public function create_reference(){
        $titan = TitanFramework::getInstance( 'clinical_cms' );
        $clinical_sitemap_create = $titan->getOption( 'clinical_sitemap_create' );
        if($clinical_sitemap_create == true){
            echo '<link rel="sitemap" type="application/xml" title="Sitemap" href="' . site_url() . '/sitemap-index.xml" />';
	...
	public function rewrite_rules() {
        ...
        if($clinical_sitemap_create == true){
            ...
            //add_rewrite_rule( 'sitemap.xml', 'index.php?alleymap=index', 'top' );
